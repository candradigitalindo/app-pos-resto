<template>
  <div class="space-y-5">
    <div class="flex items-center justify-between">
      ...
      <AppButton v-if="authStore.hasPermission('procurement.submit')" @click="openCreate">+ Buat Pengajuan</AppButton>
    </div>
      ...
      <AppTable :columns="COLUMNS" :rows="requests" :loading="loading" emptyText="Belum ada pengajuan jasa.">
        ...
        <template #cell-actions="{ row }">
          <div class="action-btns">
            ...
            <button v-if="row.status === 'pending' && authStore.hasPermission('procurement.submit')" class="act-edit" @click="viewDetail(row)" title="Edit">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
            </button>
            <button v-if="canDelete(row.status) && authStore.hasPermission('procurement.submit')" class="act-del" @click="confirmDelete(row)" title="Hapus">
    ...
    <AppModal v-model="showDetail" title="Detail Pengajuan Jasa" size="2xl">
      <div v-if="detail" class="space-y-4">
        ...
        <div v-if="!detail.children?.length || detailItems.length > 0">
          ...
          <div v-if="detail.status === 'approved' && authStore.hasPermission('procurement.purchasing')" class="mt-6">
          ...
          <div v-if="detail.status === 'pending' && authStore.hasPermission('procurement.approve')" class="mt-4 border-t border-gray-200 pt-4">
            <!-- Reject reason input -->
      ...
      <template #footer>
        <div v-if="!parentDetailId" class="flex items-center gap-2 mr-auto">
          <AppButton v-if="detail && detail.status === 'approved' && authStore.hasPermission('procurement.purchasing') && detail.total_final > 0" variant="primary" @click="confirmDetailAction('request_payment')">Ajukan Pembayaran</AppButton>
          <AppButton v-if="detail && detail.status === 'paid' && authStore.hasPermission('procurement.submit')" variant="primary" @click="confirmDetailAction('receive')">Selesaikan Jasa</AppButton>
          <AppButton v-if="detail && ['pending','approved'].includes(detail.status) && authStore.hasPermission('procurement.submit')" variant="danger" @click="confirmDetailAction('cancel')">Batalkan</AppButton>
        </div>
...
const showHps = computed(() => authStore.hasPermission('procurement.submit') || authStore.hasPermission('procurement.approve'))
