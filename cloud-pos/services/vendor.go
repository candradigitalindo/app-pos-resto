...
func ListVendors(activeOnly bool) ([]models.Vendor, error) {
	where := ""
	if activeOnly {
...
func GetVendorDetail(id string) (*models.VendorDetailResponse, error) {
	...
	// Aggregate stats from purchase_requests
	err = database.DB.QueryRow(`
		SELECT
			COUNT(*),
		...
		FROM purchase_requests WHERE vendor_id = $1
	`, id).Scan(&resp.PurchaseCount, &resp.TotalSpent, &resp.TotalPaid, &resp.TotalDebt, &resp.TotalPending)
	if err != nil {
		return nil, err
	}
...
func ListVendorPurchases(vendorID string, page, limit int) (*models.PurchaseRequestListResponse, error) {
	var total int
	if err := database.DB.QueryRow("SELECT COUNT(*) FROM purchase_requests WHERE vendor_id = $1", vendorID).Scan(&total); err != nil {
		return nil, err
	}
