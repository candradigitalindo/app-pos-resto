...
func GetDashboardStats(dateFrom, dateTo string, scopeIDs []string) (*models.DashboardStats, error) {
	...
	if scopeIDs != nil {
		...
		SELECT
			...
			(SELECT COUNT(*) FROM cloud_orders WHERE COALESCE(payment_info->>'payment_status','unpaid') NOT IN ('paid') AND payment_info->>'voided_at' IS NULL%s),
			(SELECT COALESCE(SUM(total_amount),0) FROM cloud_orders WHERE COALESCE(payment_info->>'payment_status','unpaid') NOT IN ('paid') AND payment_info->>'voided_at' IS NULL%s)
		`,
		...
		SELECT
			...
			(SELECT COUNT(*) FROM cloud_orders WHERE COALESCE(payment_info->>'payment_status','unpaid') NOT IN ('paid') AND payment_info->>'voided_at' IS NULL),
			(SELECT COALESCE(SUM(total_amount),0) FROM cloud_orders WHERE COALESCE(payment_info->>'payment_status','unpaid') NOT IN ('paid') AND payment_info->>'voided_at' IS NULL)
		`
	}
...
func GetManagerDashboard(scopeIDs []string) (*models.ManagerDashboardStats, error) {
	...
	topQuery := fmt.Sprintf(`
		SELECT p.name, COALESCE(SUM(oi.quantity), 0)::int, COALESCE(SUM(oi.subtotal), 0)
		FROM cloud_order_items oi
		JOIN cloud_products p ON p.id = oi.product_id
		JOIN cloud_orders o ON o.id = oi.order_id
		WHERE o.created_at >= date_trunc('month', CURRENT_TIMESTAMP)
		  AND o.payment_info->>'voided_at' IS NULL%s
		GROUP BY p.name ORDER BY SUM(oi.quantity) DESC LIMIT 10
	`, func() string {
		if scopeIDs != nil {
			return " AND o.outlet_id = ANY($1)"
		}
		return ""
	}())
	...
	if orErr == nil {
		...
		for orRows.Next() {
			...
			if err := orRows.Scan(&r.ID, &r.Name, &r.TodayRevenue, &r.MonthRevenue, &r.TodayOrders, &r.MonthOrders); err == nil {
