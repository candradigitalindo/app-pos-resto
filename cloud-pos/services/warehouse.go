...
func ListStockItems(search string, activeOnly bool, page, limit int) ([]models.StockItem, int, error) {
	where := "WHERE 1=1"
	args := []interface{}{}
	i := 1
	if search != "" {
	...
	var total int
	database.DB.QueryRow("SELECT COUNT(*) FROM stock_items "+where, args...).Scan(&total)
	...
	rows, err := database.DB.Query(fmt.Sprintf(`
		...
		FROM stock_items %s ORDER BY name ASC LIMIT $%d OFFSET $%d
	`, where, i, i+1), args...)
	if err != nil {
		return nil, 0, err
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.ID, &s.Code, &s.Name, &s.Category, &s.BaseUnit, &s.DistUnit,
			...
			&s.CreatedAt, &s.UpdatedAt, &s.TotalStock); err != nil {
			return nil, 0, err
		}
...
func UpdateStockItem(id string, req models.StockItemRequest) (*models.StockItem, error) {
	req.Name = strings.TrimSpace(req.Name)
	if req.Name == "" {
		return nil, fmt.Errorf("nama wajib diisi")
	}
...
func DeductStockByRecipe(outletID string, productLocalID string, qtySold float64, refID, refNumber string) error {
	...
	if stockType == "single" && linkedItemID.Valid {
		...
		for rows.Next() {
			var rID, rName, rUnit string
			var rQtyBase, rRatio float64
			if err := rows.Scan(&rID, &rQtyBase, &rName, &rUnit, &rRatio); err != nil {
				return fmt.Errorf("gagal scan bahan resep: %w", err)
			}
			
			totalQty := rQtyBase * qtySold
			if err := applyMovement(tx, rID, warehouseID, "sale", rUnit,
				refID, "transaction", refNumber, "Recipe deduction from sale", "system", -totalQty, -totalQty/rRatio, 0, ""); err != nil {
				return fmt.Errorf("failed recipe deduction for %s: %w", rName, err)
			}
		}
	}

	// 3. Update the product's simple stock field for UI reference (optional but good for consistency)
	if stockType == "single" {
		_, _ = tx.Exec(`UPDATE cloud_products SET stock = stock - $1, updated_at = NOW() WHERE id = $2`, int(qtySold), productCloudID)
	}
