          ...
          <tbody>
            ...
            <template v-else-if="products.length">
              <tr v-for="p in products" :key="p.id" class="tr-data">
                ...
                <td>
                  <span :class="['row-stock', p.stock <= 0 ? 'stock-empty' : p.stock <= 5 ? 'stock-low' : '']">{{ p.stock }}</span>
                </td>
        ...
        <div class="modal-card">
          ...
          <form class="modal-body" @submit.prevent="saveProd">
            ...
            <div class="form-row">
              ...
              <div class="form-group">
                <label>Stok</label>
                <input v-model.number="formProd.stock" class="form-input" type="number" min="0" placeholder="0" />
              </div>
            </div>
            <div class="form-group">
...
const formProd      = ref({ outlet_id: '', name: '', code: '', description: '', category_id: '', category_name: '', price: 0, stock: 0, destination: '' })
...
function openCreateProd() {
  ...
  formProd.value = { outlet_id: selectedOutlet.value, name: '', code: '', description: '', category_id: '', category_name: '', price: 0, stock: 0, destination: '' }
  priceDisplay.value = ''
...
function openEditProd(p) {
  editingProd.value = p
  formProd.value = { outlet_id: p.outlet_id, name: p.name, code: p.code ?? '', description: p.description ?? '', category_id: p.category_id ?? '', category_name: p.category_name ?? '', price: p.price, stock: p.stock, destination: p.destination ?? '' }
  ...
  try {
    if (editingProd.value) {
      await productsApi.updateProduct(editingProd.value.id, {
        ...
        price: formProd.value.price, stock: formProd.value.stock, destination: formProd.value.destination,
      })
    } else {
...
.row-stock  { font-size: .8rem; font-weight: 700; color: #0f2d1d; }
