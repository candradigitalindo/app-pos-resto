...
func runMigrations(db *sql.DB) error {
	...
	// Migration: Add is_deleted column to products table (soft delete)
	{
		var hasIsDeleted bool
		...
		if !hasIsDeleted {
			_, err := db.Exec("ALTER TABLE products ADD COLUMN is_deleted INTEGER NOT NULL DEFAULT 0")
			if err != nil {
				return err
			}
			log.Println("✅ Added is_deleted column to products table")
		}
	}

	// Migration: Add is_deleted column to categories table (soft delete)
	{
		var hasIsDeleted bool
		...
		if !hasIsDeleted {
			_, err := db.Exec("ALTER TABLE categories ADD COLUMN is_deleted INTEGER NOT NULL DEFAULT 0")
			if err != nil {
				return err
			}
			log.Println("✅ Added is_deleted column to categories table")
		}
	}
