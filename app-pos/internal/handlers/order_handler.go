...
func (h *OrderHandler) HandleGetDisplayOrders(c *echo.Context) error {
	...
	for _, order := range orders {
		...
		orderObj := db.Order{
			...
			BasketSize:    currentOrder.BasketSize,
			TotalAmount:   currentOrder.TotalAmount,
			PaidAmount:    currentOrder.PaidAmount,
			OrderStatus:   currentOrder.OrderStatus,
			CreatedBy:     currentOrder.CreatedBy,
			PaymentStatus: currentOrder.PaymentStatus,
			MergedFrom:    currentOrder.MergedFrom,
			IsMerged:      currentOrder.IsMerged,
			CreatedAt:     currentOrder.CreatedAt,
			UpdatedAt:     currentOrder.UpdatedAt,
		}
		waiterName := h.getWaiterName(ctx, &orderObj)
		mergedFromTableNumber := h.getMergedFromTableNumber(ctx, &orderObj)
		displayOrders = append(displayOrders, map[string]interface{}{
			"order": toOrderResponse(&orderObj, waiterName, mergedFromTableNumber),
			"items": filteredItems,
		})
	}
