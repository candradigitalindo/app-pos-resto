...
// FullPullFromCloud menarik semua produk & kategori dari cloud dan menimpa data lokal
func (h *SyncHandler) FullPullFromCloud(c *echo.Context) error {
	if err := h.syncService.FullPullFromCloud((*c).Request().Context()); err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{
			"error": "Gagal pull dari cloud: " + err.Error(),
		})
	}

	return c.JSON(http.StatusOK, map[string]interface{}{
		"success": true,
		"message": "Produk & kategori berhasil disinkronkan dari Cloud POS",
	})
}
