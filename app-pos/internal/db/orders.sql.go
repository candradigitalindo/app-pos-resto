...
SELECT id, table_number, customer_name, customer_phone, customer_id, pax, basket_size, total_amount, paid_amount, order_status, created_by, payment_status, merged_from, is_merged, voided_at, voided_by, void_reason, created_at, updated_at FROM orders
WHERE merged_from = ?
`
...
type GetOrdersByTableRow struct {
	ID            string         `json:"id"`
	TableNumber   string         `json:"table_number"`
	CustomerName  sql.NullString `json:"customer_name"`
	CustomerPhone sql.NullString `json:"customer_phone"`
...
func (q *Queries) GetOrdersByTable(ctx context.Context, tableNumber string) ([]GetOrdersByTableRow, error) {
	rows, err := q.db.QueryContext(ctx, getOrdersByTable, tableNumber)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetOrdersByTableRow{}
	for rows.Next() {
		var i GetOrdersByTableRow
		if err := rows.Scan(
			&i.ID,
			&i.TableNumber,
			&i.CustomerName,
...
func (q *Queries) ListOrdersByCustomer(ctx context.Context, customerID sql.NullString) ([]ListOrdersByCustomerRow, error) {
