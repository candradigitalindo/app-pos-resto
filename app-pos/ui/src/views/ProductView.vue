<template>
  <div class="min-h-screen pb-24 lg:pb-6">
      ...
      <DataTable v-else :columns="tableColumns" :data="products">
        ...
        <template #cell-actions="{ item }">
          <div class="flex items-center justify-center gap-2">
            <button @click="openAddonManager(item)" class="flex h-9 w-9 items-center justify-center rounded-lg bg-emerald-50 text-emerald-600 transition-all hover:bg-emerald-100 hover:scale-110 active:scale-95" title="Kelola Tambahan">
    ...
    <div v-if="showAddonModal" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm px-4" @click.self="showAddonModal = false">
      <div class="w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div class="bg-gradient-to-r from-emerald-600 to-emerald-500 p-5">
          <div class="flex items-center justify-between">
            <div>
              <h3 class="text-lg font-bold text-white">Kelola Tambahan</h3>
              <p class="text-sm text-emerald-100 mt-0.5">{{ addonProduct?.name }}</p>
            </div>
            <button @click="showAddonModal = false" class="flex h-9 w-9 items-center justify-center rounded-lg bg-white/20 text-white transition hover:bg-white/30">
        ...
        <div class="p-5 space-y-4">
          ...
          <div class="flex gap-2">
            <input v-model="newAddonName" type="text" placeholder="Nama tambahan" class="flex-1 h-10 px-3 rounded-lg border border-slate-200 text-sm focus:outline-none focus:border-emerald-400" />
            <input :value="newAddonPriceDisplay" @input="formatNewAddonPrice" @blur="formatNewAddonPriceBlur" type="text" inputmode="numeric" placeholder="Rp 0" class="w-32 h-10 px-3 rounded-lg border border-slate-200 text-sm focus:outline-none focus:border-emerald-400" />
            <button @click="createAddon" :disabled="!newAddonName.trim() || savingAddon" class="h-10 px-4 rounded-lg bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-700 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed shrink-0">
              <svg v-if="savingAddon" class="h-4 w-4 animate-spin" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" fill="none"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>
              <span v-else>Tambah</span>
            </button>
          </div>
          <!-- Addon list -->
          <div v-if="loadingAddons" class="py-8 text-center text-slate-400">
            <div class="h-6 w-6 animate-spin rounded-full border-2 border-emerald-600 border-t-transparent mx-auto mb-2"></div>
            Memuat...
          </div>
          <div v-else-if="addonList.length === 0" class="py-8 text-center text-slate-400">
            <svg class="h-8 w-8 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>
            <p class="text-sm">Belum ada tambahan</p>
          </div>
          <div v-else class="space-y-2 max-h-64 overflow-y-auto">
            <div v-for="addon in addonList" :key="addon.id" class="flex items-center gap-3 rounded-xl bg-slate-50 p-3 border border-slate-200">
              <div v-if="editingAddon === addon.id" class="flex-1 flex gap-2">
                <input v-model="editAddonName" type="text" class="flex-1 h-8 px-2.5 rounded-lg border border-slate-200 text-sm focus:outline-none focus:border-emerald-400" />
                <input :value="editAddonPriceDisplay" @input="formatEditAddonPrice" type="text" inputmode="numeric" class="w-28 h-8 px-2.5 rounded-lg border border-slate-200 text-sm focus:outline-none focus:border-emerald-400" />
                <button @click="updateAddon(addon.id)" class="h-8 px-3 rounded-lg bg-emerald-600 text-white text-xs font-semibold hover:bg-emerald-700 active:scale-95">Simpan</button>
                <button @click="editingAddon = null" class="h-8 px-3 rounded-lg bg-slate-200 text-slate-600 text-xs font-semibold hover:bg-slate-300 active:scale-95">Batal</button>
              </div>
              <template v-else>
                <div class="flex-1 min-w-0">
                  <div class="text-sm font-semibold text-slate-800">{{ addon.name }}</div>
                  <div class="text-xs text-emerald-600 font-bold">+Rp {{ formatRupiah(addon.price) }}</div>
                </div>
                <button @click="startEditAddon(addon)" class="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 active:scale-95">
                  <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                </button>
                <button @click="deleteAddon(addon.id)" class="flex h-8 w-8 items-center justify-center rounded-lg bg-red-50 text-red-600 hover:bg-red-100 active:scale-95">
...
const newAddonPrice = ref(0)
const newAddonPriceDisplay = ref('')
const editingAddon = ref(null)
const editAddonName = ref('')
const editAddonPrice = ref(0)
const editAddonPriceDisplay = ref('')
...
// Options for the category filter SearchSelect
...
// === Addon Management ===
const openAddonManager = async (product) => {
  addonProduct.value = product
  showAddonModal.value = true
  newAddonName.value = ''
  newAddonPrice.value = 0
  newAddonPriceDisplay.value = ''
...
const fetchAddons = async (productId) => {
  ...
  try {
    ...
    addonList.value = response.data.data || []
  } catch (err) {
...
const createAddon = async () => {
  if (!newAddonName.value.trim() || !addonProduct.value) return
  savingAddon.value = true
  try {
    await api.post(`/products/${addonProduct.value.id}/addons`, {
      name: newAddonName.value.trim(),
      price: newAddonPrice.value || 0
    })
    newAddonName.value = ''
    newAddonPrice.value = 0
    newAddonPriceDisplay.value = ''
    await fetchAddons(addonProduct.value.id)
    created('Tambahan berhasil ditambahkan')
  } catch (err) {
    error('Gagal menambahkan tambahan')
  } finally {
    savingAddon.value = false
  }
}

const formatNewAddonPrice = (event) => {
  let value = event.target.value.replace(/[^\d]/g, '')
  if (value === '') {
    newAddonPriceDisplay.value = ''
    newAddonPrice.value = 0
  ...
  newAddonPrice.value = num
  newAddonPriceDisplay.value = 'Rp ' + num.toLocaleString('id-ID')
}

const formatNewAddonPriceBlur = () => {
  if (!newAddonPrice.value) {
    newAddonPriceDisplay.value = ''
  }
}

const formatEditAddonPrice = (event) => {
  let value = event.target.value.replace(/[^\d]/g, '')
  if (value === '') {
    editAddonPriceDisplay.value = ''
    editAddonPrice.value = 0
    return
  }
  const num = parseInt(value, 10)
  editAddonPrice.value = num
  editAddonPriceDisplay.value = 'Rp ' + num.toLocaleString('id-ID')
}

const startEditAddon = (addon) => {
  editingAddon.value = addon.id
  editAddonName.value = addon.name
  editAddonPrice.value = addon.price
  editAddonPriceDisplay.value = addon.price > 0 ? 'Rp ' + addon.price.toLocaleString('id-ID') : ''
}

const updateAddon = async (addonId) => {
  try {
    await api.put(`/products/addons/${addonId}`, {
      name: editAddonName.value.trim(),
      price: editAddonPrice.value || 0
    })
    editingAddon.value = null
    await fetchAddons(addonProduct.value.id)
    updated('Tambahan berhasil diperbarui')
  } catch (err) {
    error('Gagal memperbarui tambahan')
  }
}
...
const deleteAddon = async (addonId) => {
  try {
    ...
    await fetchAddons(addonProduct.value.id)
    deleted('Tambahan berhasil dihapus')
  } catch (err) {
    error('Gagal menghapus tambahan')
  }
}
