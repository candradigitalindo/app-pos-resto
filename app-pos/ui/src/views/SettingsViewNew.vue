        ...
        <div class="space-y-6">
          <div v-if="activeSection === 'outlet'" class="card space-y-4">
            ...
            <div>
              ...
              <input :value="spendPerPaxDisplay" @input="onSpendPerPaxInput" type="text" inputmode="numeric" class="input" placeholder="Rp 75.000" :disabled="!isEditingOutlet" />
              ...
              <div v-if="isEditingCharge && !syncStatus.sync_enabled" class="space-y-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <input
                      v-if="chargeForm.charge_type === 'fixed'"
                      :value="chargeValueDisplay"
                      @input="onChargeValueInput"
                      type="text"
                      inputmode="numeric"
                      class="input"
                    ...
                    <input
                      v-else
                      v-model.number="chargeForm.value"
                      type="number"
                      min="0"
                      max="100"
                      class="input"
...
const spendPerPaxDisplay = ref('')
function onSpendPerPaxInput(e) {
  const num = parseRupiahInput(e.target.value)
  config.value.target_spend_per_pax = num
  spendPerPaxDisplay.value = num ? fmtRupiahInput(num) : ''
}
...
function onChargeValueInput(e) {
  ...
  chargeValueDisplay.value = num ? fmtRupiahInput(num) : ''
}

// When charge_type switches to percentage, reset display
watch(() => chargeForm.value.charge_type, (newType) => {
  if (newType === 'percentage') {
    chargeValueDisplay.value = ''
  } else {
    chargeValueDisplay.value = chargeForm.value.value ? fmtRupiahInput(chargeForm.value.value) : ''
  }
})
...
const startEditCharge = (charge) => {
  ...
  chargeValueDisplay.value = (charge.charge_type === 'fixed' && charge.value) ? fmtRupiahInput(charge.value) : ''
  isEditingCharge.value = true
  errorMessage.value = ''
  successMessage.value = ''
}
